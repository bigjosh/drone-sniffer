...
// IDs are CTA-2063 ASCII; anything unprintable or JSON-special becomes '?'.
static void json_safe_id(const char *in, char *out, size_t outsz) {
  size_t j = 0;
  for (size_t i = 0; in[i] != '\0' && j + 1 < outsz; i++) {
    char c = in[i];
    if (c < 0x20 || c == '"' || c == '\\' || c == 0x7F) c = '?';
    out[j++] = c;
  }
  out[j] = '\0';
}

// One JSON line per detection to the display node. Rate-limited per drone
// (not globally) so several drones can share the link without starving
// each other.
void print_compact_message(const id_data *UAV) {
  static struct { uint8_t mac[6]; unsigned long t; } sent[MAX_UAVS] = {0};
  const unsigned long sendInterval = 1000;
  unsigned long now = millis();

  int slot = -1;
  for (int i = 0; i < MAX_UAVS; i++) {
    if (memcmp(sent[i].mac, UAV->mac, 6) == 0) { slot = i; break; }
  }
  if (slot >= 0) {
    if (now - sent[slot].t < sendInterval) return;
  } else {
    for (int i = 0; i < MAX_UAVS; i++) {
      if (sent[i].t == 0) { slot = i; break; }
    }
    if (slot < 0) slot = 0;
    memcpy(sent[slot].mac, UAV->mac, 6);
  }
  sent[slot].t = now ? now : 1;

  char mac_str[18];
  snprintf(mac_str, sizeof(mac_str), "%02x:%02x:%02x:%02x:%02x:%02x",
           UAV->mac[0], UAV->mac[1], UAV->mac[2],
           UAV->mac[3], UAV->mac[4], UAV->mac[5]);
  char id_str[ODID_ID_SIZE + 1];
  json_safe_id(UAV->uav_id, id_str, sizeof(id_str));
  ...
  if (len > 0 && len < (int)sizeof(msg) && UAV->base_lat_d != 0.0 && UAV->base_long_d != 0.0) {
    len += snprintf(msg + len, sizeof(msg) - len, ",\"plat\":%.6f,\"plon\":%.6f",
...
#ifdef TEST_EMIT
  // Fake detections on the display link, cycling three synthetic drones.
  static unsigned long last_test_emit = 0;
  static int test_seq = 0;
  if (current_millis - last_test_emit > 3000) {
    last_test_emit = current_millis;
    char msg[240];
    int rssi = -50 - (test_seq % 40);
    switch (test_seq % 3) {
      case 0:
        snprintf(msg, sizeof(msg),
                 "{\"id\":\"TESTDRONE0001\",\"mac\":\"aa:bb:cc:00:00:01\",\"rssi\":%d,\"tp\":\"ble\"}",
                 rssi);
        break;
      case 1:
        snprintf(msg, sizeof(msg),
                 "{\"id\":\"TESTDRONE0002\",\"mac\":\"aa:bb:cc:00:00:02\",\"rssi\":%d,\"tp\":\"wifi\","
                 "\"dlat\":38.897700,\"dlon\":-77.036500,\"plat\":38.889500,\"plon\":-77.035200}",
                 rssi);
        break;
      default:
        snprintf(msg, sizeof(msg),
                 "{\"id\":\"\",\"mac\":\"aa:bb:cc:00:00:03\",\"rssi\":%d,\"tp\":\"wifi\"}",
                 rssi);
        break;
    }
    Serial1.println(msg);
    Serial.printf("[TEST_EMIT] %s\n", msg);
    test_seq++;
  }
#endif
}
